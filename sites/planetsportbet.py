...
class PlanetSportBetAdapter(BaseSiteAdapter):
    ...
    def fill_registration(self, page: Page, client: Client, password: str) -> RegistrationResult:
        ...
        try:
            ...
            # 5. Polling Loop (up to 30s) to wait for Planet Sport Bet confirmation & auto-verification
            log.info("Waiting for Planet Sport Bet in-platform confirmation (up to 30s)...")
            max_poll_sec = 30
            is_confirmed = False
            ...
            for sec in range(1, max_poll_sec + 1):
                ...
                if has_auth or not agree_still_open:
                    log.info(f"Planet Sport Bet registration confirmed at {sec}s!")
                    is_confirmed = True
                    break
            ...
            if is_confirmed:
                ...
                return RegistrationResult(
                    ...
                    password=password,
                    account_reference="PlanetSportBet-Direct",
                    screenshot_path=success_shot
                )
            else:
                bundle = capture_failure_bundle(page, client.client_id, self.site_id, "verify_submission")
                log.warning("Planet Sport Bet submission could not be confirmed within 30s")
                return RegistrationResult(
                    client_id=client.client_id,
                    client_name=client.full_name,
                    site_id=self.site_id,
