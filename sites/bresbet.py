...
class BresbetAdapter(BaseSiteAdapter):
    """Adapter for BresBet (https://bresbet.com/)."""

    def __init__(self, promo_url: str = "https://bresbet.com/?promo=welcomeb10g10&keyword=bresbet&btag=affelios"):
        super().__init__(
            site_id="bresbet",
            site_name="BresBet",
    ...
    def fill_registration(self, page: Page, client: Client, password: str) -> RegistrationResult:
        ...
        log.info(f"Starting BresBet registration for {client.full_name}")

        try:
            # 1. Direct navigation to clean signup URL or trigger modal
            cur_url = page.url if isinstance(getattr(page, "url", None), str) else ""
            if "?account=signup" not in cur_url and "?promo=" not in cur_url:
                try:
                    page.goto("https://bresbet.com/?account=signup", wait_until="domcontentloaded", timeout=25000)
            ...
            if step1_err.is_visible(timeout=1500):
                ...
                if is_already_registered_error(err_txt) or any(kw in err_txt.lower() for kw in ["already exists", "in use", "already registered", "taken"]):
                    log.warning(f"BresBet: Client {client.full_name} is ALREADY REGISTERED ({err_txt})")
                    ...
                    return RegistrationResult(
                        ...
                        email=client.email,
                        password=password,
                        account_reference="BresBet-Existing",
            ...
            if not fn_inp.is_visible(timeout=3000):
                ...
                if dup_err.is_visible(timeout=1000):
                    ...
                    if is_already_registered_error(raw_txt) or "already exists" in raw_txt.lower():
                        log.warning(f"BresBet: Client {client.full_name} is ALREADY REGISTERED ({clean_err})")
                        bundle = capture_failure_bundle(page, client.client_id, self.site_id, "already_registered")
                        return RegistrationResult(
                            client_id=client.client_id,
                            client_name=client.full_name,
            ...
            if title_el.is_visible(timeout=2000):
                log.info(f"Selecting '{title_text}' title on BresBet")
            ...
            if error_el.is_visible(timeout=3000):
                ...
                if is_duplicate:
                    log.warning(f"[DUPLICATE] BresBet: Client {client.full_name} is ALREADY REGISTERED ({final_err_msg})")
                    bundle = capture_failure_bundle(page, client.client_id, self.site_id, "already_registered")
                    return RegistrationResult(
                        client_id=client.client_id,
                        client_name=client.full_name,
                    ...
                    log.warning(f"BresBet registration rejected: {final_err_msg}")
            ...
            log.info("Waiting for BresBet onboarding & in-platform confirmation (up to 35s)...")
            max_poll_sec = 35
            is_confirmed = False
            has_auth = False
            ...
            for sec in range(1, max_poll_sec + 1):
                ...
                if handle_playbook_deposit_step(page, log if sec % 5 == 1 else None):
                    log.info(f"BresBet: Pressed SKIP on deposit step at {sec}s")
                    page.wait_for_timeout(1000)
                ...
                if handle_playbook_deposit_step(page, log if sec % 5 == 1 else None):
                    log.info(f"BresBet: Pressed SKIP on deposit step following Safer Gambling at {sec}s")
                    page.wait_for_timeout(1000)
                ...
                if (has_auth or sec > 5) and not is_onboarding_open:
                    log.info(f"BresBet onboarding dismissed and session confirmed at {sec}s!")
                    is_confirmed = True
                    break
            ...
            if is_confirmed and not is_onboarding_open:
                log.info("BresBet registration and onboarding confirmed successfully!")
                ...
                return RegistrationResult(
                    ...
                    username=client.email,
                    password=password,
                    account_reference="BresBet-Direct (Active Session Verified)" if has_auth else "BresBet-Direct",
                    screenshot_path=success_shot,
                ...
                err_msg = "BresBet safer gambling onboarding modal could not be dismissed" if is_onboarding_open else "BresBet submission could not be confirmed within 35s"
                log.warning(err_msg)
                return RegistrationResult(
                    client_id=client.client_id,
                    client_name=client.full_name,
            ...
            log.error(f"Registration error on BresBet: {e}")
    ...
    def login(
        ...
        """Specialized login verification handler for BresBet."""
        cid = client_id or "client"
        log = get_logger(client_id=cid, site_id=self.site_id, step="login")
        log.info(f"Navigating to BresBet clean login URL: https://bresbet.com/?account=login")

        try:
            page.goto("https://bresbet.com/?account=login", wait_until="domcontentloaded", timeout=25000)
            page.wait_for_timeout(2000)
            ...
            if not email_inp.is_visible(timeout=4000) or not pwd_inp.is_visible(timeout=4000):
                ...
                return False, bundle.screenshot_path, "BresBet login inputs not visible"
