...
- **🚀 Phase 2 Bookmakers & Human-Like Behavioral Automation:**
  ...
  - **BetTOM (`https://www.bettom.com/en/sport/`):** Full EveryMatrix Polymer shadow DOM integration with Cybot Cookiebot bypass, dynamic UK personal title selection (`Mr.`, `Mrs.`, `Miss`, `Ms.`), ISO DOB handling, address matching, safe 18+ declaration, pre-submission name validation self-healing, instant detection of the `CONGRATULATIONS!` modal (*"Your account was successfully registered!"* / *GO TO LOGIN*) terminating verification wait immediately upon appearance, active post-submission verification polling, and Step 2 Marketing Preferences automation.
  ...
  - **247 Bet (`https://www.247bet.com/en-gb/register`):** Multi-step White Hat Gaming UK account creation covering:
    - **Step 1 (`/create-account/personal-details`):** Email, sanitized phone number, username, password with active duplicate registration error detection.
    - **Step 2 (`/create-account/address-details`):** Personal gender radio selection, lowercase First/Last name input binding (`#firstname`, `#lastname`), select dropdown Date of Birth population (`select#day`, `select#month`, `select#year`), automated expansion of manual address entry (`Enter address manually`), and Angular reactive form controls (`address1`, `town`, `postalcode`).
    - **Step 3 (`/create-account/account-details`):** Safe box targeting on mandatory terms and funds policy declaration (`input#termsPrivacyFunds`), marketing preference handling, and final submission (`button.btn-primary:has-text('Continue')`).
    - **Post-Submission Polling & Session Verification:** Actively waits for redirection away from `/create-account/` or appearance of deposit/cashier/account components rather than blind early exit.
    - **Dedicated Login Verification:** Bypasses off-screen responsive drawer elements (`x < 0`) by specifically targeting the visible top navbar Login CTA (`a.btn--login:visible`), fills modal credentials (`txt-login-username`, `txt-login-password`), and captures logged-in proof.
...
- **UK Data Normalization & Name Safeguards (`data/models.py`):**
  - **Automated First & Last Name Sanitization:**
    - Strips title prefixes (`Mr.`, `Mrs.`, `Miss`, `Ms.`, `Dr.`) if entered into the first name field and extracts the primary first name.
    - Resolves middle names placed in surname columns (e.g. `kay Samuda` $\rightarrow$ `Samuda`) to preserve the client's legal surname and match credit bureau / electoral roll verification databases.
    - Automatically joins recognized compound surname particles with hyphens (e.g. `St John` $\rightarrow$ `St-John`, `Van Der Bilt` $\rightarrow$ `Van-Der-Bilt`).
    - Provides strict alphabetic helper properties (`alphabetic_first_name`, `alphabetic_last_name`) for platforms rejecting spaces and special symbols.
  - **Active Self-Healing Validation on BetTOM (`sites/bettom.py`):**
    - Directly guards against EveryMatrix / BetTOM inline field rejection (`"Only alphabetic characters are allowed."`).
    - Pre-emptively sanitizes input and performs real-time detection immediately after typing. If an inline error ever appears, executes an automatic self-healing routine (clearing and re-typing strictly alphabetic characters) before proceeding.
    - Performs an additional pre-submission safety sweep before clicking final submission (`DONE`).
